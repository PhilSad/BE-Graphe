...
public class AStarAlgorithm extends DijkstraAlgorithm {
    ...
    protected ShortestPathSolution doRun() {
    	...
    	HashMap<Integer,LabelStar> hashmap = new HashMap<Integer, LabelStar>();        
        Node origin = data.getOrigin();
        Node destination = data.getDestination();
        boolean fin = false;
        
        /* On ajoute le sommet de départ dans le tas */
        LabelStar LabelStarOrigin = new LabelStar(origin, destination);
        hashmap.put(origin.getId(), LabelStarOrigin);
        LabelStarOrigin.setCost(0.0);
        tas.insert(LabelStarOrigin);

        notifyOriginProcessed(origin); // notify origin processed
        
        /* Algorithme de Dijkstra */
        while(!tas.isEmpty() && !fin) {
        	LabelStar x = (LabelStar) tas.deleteMin();
        	x.setMarque(true);
        	...
        	for(Arc a : x.getSommetCourant().getSuccessors()) {
    			...
    			*/        		
                ...
                // on recupere le LabelStar si il existe, sinon on le crée
                LabelStar y = hashmap.get(a.getDestination().getId());
                if(y == null) {
                	y = new LabelStar(a.getDestination(), destination);
        		...
        		if(!y.isMarque()) {
        			...
        			if(newCost < y.getCost()) {           				
        				...
        				//on supprime et rajoute le LabelStar pour mettre à jour sa position
        				tas.remove(y);
        				tas.insert(y);
        			}        			
        		}
        ...
        LabelStar curLabelStar = hashmap.get(destination.getId());
        if(curLabelStar == null) {
        	curLabelStar = new LabelStar(destination, destination);
        	hashmap.put(destination.getId(), curLabelStar);
        }
        
        if(curLabelStar.getPere() == null) {
        	...
        	do {
        		pathNodes.add(curLabelStar.getSommetCourant());
        		curLabelStar = hashmap.get(curLabelStar.getPere().getId());
        	}while(!curLabelStar.getSommetCourant().equals(origin));
